...
- Use Bulma with BEM-oriented structure; keep templates readable (avoid long class chains). Prefer semantic Bulma classes combined with small, component-level scss extensions, and avoid duplicating layout logic inside templates.
...
- Keep spacing and colors consistent (use Bulma spacing helpers and its design system together with existing design tokens; avoid ad-hoc pixel values unless absolutely necessary).
